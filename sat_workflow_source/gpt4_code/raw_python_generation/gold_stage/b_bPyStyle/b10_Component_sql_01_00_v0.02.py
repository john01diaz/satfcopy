...
def filter_dataframe_by_class(
        dataframe):
    class_filter_values = ['Instrumentation', 'Inst(Shared)', 'Elec(Shared)']
    return dataframe[dataframe[S_Component.CLASS.value].isin(
        class_filter_values)]


def filter_dataframe_by_database_name(
        dataframe,
        valid_database_names):
    return dataframe[dataframe[S_Component.DATABASE_NAME.value].isin(
        valid_database_names)]
...
def add_sequence_column(
        dataframe):
    dataframe[Component.SEQUENCE.value] = dataframe.groupby(
            [Component.PARENT_EQUIPMENT_NO.value,
...
def convert_sql_to_pandas(
    ...
    filtered_dataframe = filter_dataframe_by_class(
        dataframe)
    valid_database_names = database_names_dataframe['Database_name']
    filtered_dataframe = filter_dataframe_by_database_name(
        filtered_dataframe,
        valid_database_names)
    distinct_dataframe = select_distinct_columns(
        filtered_dataframe)
    return add_sequence_column(
        distinct_dataframe)
...
output_dataframe = convert_sql_to_pandas(
    Component_dataframe,
