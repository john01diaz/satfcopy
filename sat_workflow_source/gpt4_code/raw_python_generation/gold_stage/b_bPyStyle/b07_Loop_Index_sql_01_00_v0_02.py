...
def loop_index(
        dataframe_loop_index,
        dataframe_database_names,
        column_filters,
        column_mappings,
        constant_columns):
    constant_columns = \
    ...
    filtered_dataframe = \
        filter_dataframe_by_columns(dataframe_loop_index, column_filters)

    mapped_dataframe = \
        map_dataframe_columns(filtered_dataframe, column_mappings)

    constant_added_dataframe = \
        add_constant_columns(mapped_dataframe, constant_columns)

    filtered_class_dataframe = \
        constant_added_dataframe[constant_added_dataframe['Class'] == 'Instrumentation']

    database_names = \
        dataframe_database_names['Database_name'].unique()

    final_dataframe = \
        filtered_class_dataframe[
            filtered_class_dataframe['database_name'].isin(database_names)].drop_duplicates()
