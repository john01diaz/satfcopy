...
def get_instrument_index(
		database_names_csv: str,
		cable_schedule_csv: str) -> pd.DataFrame:
	database_names = get_database_names(
		database_names_csv)
	cable_schedule_data = get_cable_schedule_data(
		cable_schedule_csv,
		database_names)
	...
	if column_renames:
		cable_schedule_data = cable_schedule_data.rename(
			columns=column_renames)

	# Make the DataFrame immutable
	cable_schedule_data = cable_schedule_data.set_flags(
		writeable=False)
...
def get_cable_schedule_data(
		cable_schedule_csv: str,
		database_names: Tuple[str]) -> pd.DataFrame:
	cable_schedule_data = pd.read_csv(
		cable_schedule_csv)

	# Filter the data based on the database_names
	cable_schedule_data = cable_schedule_data[cable_schedule_data['database_name'].isin(
		database_names)]
