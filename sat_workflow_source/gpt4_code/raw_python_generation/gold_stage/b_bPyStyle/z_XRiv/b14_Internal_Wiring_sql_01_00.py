...
def filter_classes(
		data_frame,
		classes):
	return data_frame[data_frame['Class'].isin(
		classes)]
...
def filter_database_names(
		data_frame,
		database_names):
	return data_frame[data_frame['database_name'].isin(
		database_names)]
...
def internal_wiring(
	...
	filtered_data = filter_classes(
		input_data,
		CLASSES)

	# Extract unique database names from the database_names_data dataframe
	unique_database_names = set(
		database_names_data['Database_name'].unique())

	# Filter data based on unique database names
	filtered_data = filter_database_names(
		filtered_data,
		unique_database_names)

	# Select required columns
	selected_columns_data = filtered_data[list(
