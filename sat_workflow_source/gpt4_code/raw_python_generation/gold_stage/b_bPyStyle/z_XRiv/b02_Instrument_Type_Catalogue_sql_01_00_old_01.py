...
def filter_instrument_data(
		data_frame: pd.DataFrame) -> pd.DataFrame:
	conditions = (
			data_frame['CS_location_full_designation'].isnull()
			& data_frame['loop_element_database_name'].isin(
		database_names)  # Assuming database_names is defined
...
def get_unique_instrument_types(
		data_frame: pd.DataFrame) -> pd.DataFrame:
	data_frame['Type'] = data_frame['CS_loop_element_id'].apply(
		extract_type)
...
def process_instrument_data(
		input_file_path: str,
		output_file_path: str):
	input_data = read_data_from_file(
		input_file_path)
	filtered_instrument_data = filter_instrument_data(
		input_data)
	unique_instrument_types = get_unique_instrument_types(
		filtered_instrument_data)
	sorted_instrument_types = sort_by_type(
