...
def merge_dataframes(
		df1: pd.DataFrame,
		df2: pd.DataFrame,
		on_columns: Dict[str, str]) -> pd.DataFrame:
	return pd.merge(
		df1,
		df2,
		left_on=list(
...
def cable_core_catalogue(
		s_cable_core_catalogue_csv: str,
		s_cable_catalogue_number_master_csv: str,
		s_cable_catalogue_csv: str,
	...
	filtered_s_cable_catalogue = s_cable_catalogue[s_cable_catalogue['Class'].isin(
		['Instrumentation', 'Inst(Shared)', 'Elec(Shared)'])]

	# Merge dataframes
	merged_data = merge_dataframes(
		s_cable_core_catalogue,
		s_cable_catalogue_number_master,
		{
	...
	filtered_merged_data = merged_data[merged_data['database_name'].isin(
		database_names)]

	# Add Dense Rank
	filtered_merged_data['RNT'] = filtered_merged_data.groupby(
		'CatalogueNo')['database_name', 'Cable_Object_Identifier'].rank(
		method='dense',
		ascending=True)
	...
	# Select columns
	selected_columns = final_data[
		['CatalogueNo', 'Description', 'Group_Marking', 'Group_Marking_Sequence', 'Core_Markings',
