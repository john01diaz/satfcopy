...
def filter_by_class_and_database(
		terminations_df,
		database_names_df):
	filtered_df = terminations_df[
		terminations_df["Class"].isin(
			["Instrumentation", "Inst(Shared)", "Elec(Shared)"])
	]
	database_name_filtered_df = database_names_df["Database_name"]

	return filtered_df[filtered_df["database_name"].isin(
		database_name_filtered_df)]
...
def terminations_13():
	...
	filtered_df = filter_by_class_and_database(
		terminations_df,
		database_names_df)
	distinct_columns_df = get_distinct_columns(
		filtered_df)

	# Apply column renames using the RenamedColumns class
	column_rename_dict = {}  # Add any required renames in this dictionary
	renamed_columns = RenamedColumns(
		column_rename_dict)
	final_df = renamed_columns.apply(
		distinct_columns_df)
