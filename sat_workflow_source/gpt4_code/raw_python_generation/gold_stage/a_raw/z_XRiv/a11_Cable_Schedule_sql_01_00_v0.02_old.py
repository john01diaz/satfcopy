...
def get_unique_rows(df: pd.DataFrame) -> pd.DataFrame:
    return df.drop_duplicates()

def get_rows_by_database_name(df: pd.DataFrame, database_names: List[str]) -> pd.DataFrame:
    return df[df['database_name'].isin(database_names)]

def rename_columns(df: pd.DataFrame, column_renames: dict) -> pd.DataFrame:
    return df.rename(columns=column_renames)

def _07_Loop_Index(cable_schedule_file: str, database_names_file: str) -> pd.DataFrame:
    cable_schedule_df = read_csv(cable_schedule_file)
    database_names_df = read_csv(database_names_file)
    
    cable_schedule_df = get_unique_rows(cable_schedule_df)
    
    database_names = database_names_df['Database_name'].tolist()
    cable_schedule_filtered_df = get_rows_by_database_name(cable_schedule_df, database_names)
    
    cable_schedule_final_df = rename_columns(cable_schedule_filtered_df, COLUMN_RENAMES)
