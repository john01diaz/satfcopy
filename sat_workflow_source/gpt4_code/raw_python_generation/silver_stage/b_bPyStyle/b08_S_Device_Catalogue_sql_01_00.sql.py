...
def coalesce(
    ...
    """
    for arg in args:
        if arg is not None and not pandas.isna(
                arg):
            return arg
    return None
...
def replace_string(
    ...
    """
    if value is not None and not pandas.isna(
            value):
    ...
    return coalesce(
            '-'.join(
                    [row[S_ItemFunction.DESCRIPTION.value],
                     ...
                     replace_string(
                             row[S_ItemFunction.DATABASE_NAME.value],
                             '_2016R3',
                             '')]),
            'RHLDD'
            )
...
def process_dataframe(
    ...
    joined_dataframe = joined_dataframe[
        joined_dataframe[S_ItemFunction.TYPE.value].isin(
                ['Device', 'FTA']) &
        joined_dataframe[S_ItemFunction.LOCATION_DESIGNATION.value].notna() &
        (coalesce(
                joined_dataframe[S_Item_Function_Model.LEFT.value],
                0) +
         coalesce(
                 joined_dataframe[S_Item_Function_Model.RIGHT.value],
                 0)) > 0
        ].copy()
    
    joined_dataframe[S_DeviceCatalogue.ALLOW_USE.value] = 'TRUE'
    joined_dataframe[S_DeviceCatalogue.DESCRIPTION.value] = joined_dataframe.apply(
        calculate_description,
        axis=1)
    joined_dataframe[S_DeviceCatalogue.CATALOGUE_RNT.value] = joined_dataframe.groupby(
            S_Item_Function_Model.MODELNO.value)[S_ItemFunction.ITEM_OBJECT_IDENTIFIER.value].rank(
        method="first")
