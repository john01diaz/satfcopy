...
def process_terminations(
    ...
    final_result = pd.concat(
            [first_result, second_result]).sort_values(
            [S_Connection.DATABASE_NAME.value, S_Connection.OBJECT_IDENTIFIER.value])
    return final_result
...
principles
you
've requested. It uses pandas dataframe operations instead of SQL operations and is functionally equivalent to the SQL query you provided. You can use the process_terminations function by providing the three dataframes as parameters. The function will return the final result dataframe.
