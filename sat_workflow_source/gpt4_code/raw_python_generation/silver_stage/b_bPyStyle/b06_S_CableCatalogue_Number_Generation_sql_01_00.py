...
def generate_cable_catalogue_number_master(
    ...
    merged_dataframe[S_CableCatalogueColumns.DESCRIPTION.value] = merged_dataframe[
    ...
    merged_dataframe[MARKINGS] = merged_dataframe.groupby(
    ...
    catalogue_master_dataframe = catalogue_master_dataframe.sort_values(
        by=[S_CableCatalogueColumns.DESCRIPTION.value, MARKINGS])
    catalogue_master_dataframe[RANK] = catalogue_master_dataframe.groupby(
        [S_CableCoreCatalogueColumns.DATABASE_NAME.value]).cumcount() + 2000
    catalogue_master_dataframe[S_CableCatalogueNumberMasterColumns.CATALOGUE_NO.value] = CATALOGUE_NO_PREFIX + \
                                                                                         catalogue_master_dataframe[
                                                                                             RANK].astype(
                                                                                             str)
    
    catalogue_master_dataframe = catalogue_master_dataframe.drop(
