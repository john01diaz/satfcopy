...
def create_loop_index_dataframe(
		cable_schedule_dataframe: pandas.DataFrame,
		vw_database_names_dataframe: pandas.DataFrame):
	# cable_schedule_df = read_csv_to_dataframe(
	# 	cable_schedule_csv)
	# vw_database_names_df = read_csv_to_dataframe(
	# 	vw_database_names_csv)

	database_names = vw_database_names_dataframe['Database_name'].tolist()
	filtered_cable_schedule = cable_schedule_dataframe[cable_schedule_dataframe['database_name'].isin(
		database_names)]
	...
	renamed_df = \
		__rename_columns(
			filtered_cable_schedule,
			column_mapping)

	return \
		renamed_df.drop_duplicates()
