...
def cleaning_df_columns_with_enum_display (
    ...
    columns_needed_list = ["_dyn_class","_href","BS_Instance","_enum","_VALUE","-name","_scope"]
    ...
    for column_name in df.columns:
        ...
        if column_name.endswith("Function_occ"):
            data_type = dict(df.dtypes)[column_name]
            if data_type == "string":
                df = df.withColumn(column_name, from_json(col(column_name), static_schema, {"ignoreNullFields" : "false"}))
        ...
        if "parsed_allocation" in column_name:  
            ...
            elif column_name.endswith('_scope'):
                column_rename = (column_name.split("-")[-2] + column_name.split("-")[-1])
