...
def create_silver_08_s_device_catalogue_dataframe(s_itemfunction_dataframe, s_item_function_model_dataframe):
    ...
    merged_dataframe = merged_dataframe[
        ...
        ((merged_dataframe[S_Item_Function_Model.LEFT.value].fillna(0) + merged_dataframe[S_Item_Function_Model.RIGHT.value].fillna(0)) > 0)
    ]

    # Step 3: Create new derived columns and perform transformations
    merged_dataframe[S_DeviceCatalogue.ALLOW_USE.value] = "TRUE"
    merged_dataframe[S_DeviceCatalogue.TYPE.value] = merged_dataframe[S_ItemFunction.DEVICE_TYPE.value],
    ...
    merged_dataframe[S_DeviceCatalogue.CATALOGUE_RNT.value] = merged_dataframe.groupby(S_Item_Function_Model.MODELNO.value)[S_ItemFunction.ITEM_OBJECT_IDENTIFIER.value].rank(method='first')
